...
function displayForecast(response){
  ...
  forecast.forEach(function(forecastDay, index){if (index<5) {
    ...
    <span class="weather-forecast-temperature-max">max${Math.round(forecastDay.temp.max)}°C</span>
    <span class="weather-forecast-temperature-min">min${Math.round(forecastDay.temp.min)}°C</span>
    </div>
    </div>
    </div>
    `;}
